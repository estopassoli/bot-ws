...
const {
    desbloquearComandosGlobal,
    bloquearComandosGlobal
} = require("../lib/bloqueioComandos")
...
const {
    botAlterarLimitador,
    botInfo,
    botAlterarLimiteDiario,
    botQtdLimiteDiario,
    botAlterarLimitarMensagensPv,
    botAlterarAutoSticker,
    botAlterarAntitrava,
    botAlterarPvLiberado
} = require('../lib/bot')
    ...
    try {
        ...
        switch (command) {
            ...
            case "!bcmdglobal":
                ...
                var usuarioComandos = body.slice(12).split(" "),
                    respostaBloqueio = await bloquearComandosGlobal(usuarioComandos)
            ...
            case "!dcmdglobal":
                if (args.length === 1) return await client.reply(from, erroComandoMsg(command), id)
                var usuarioComandos = body.slice(12).split(" "),
                    respostaDesbloqueio = await desbloquearComandosGlobal(usuarioComandos)
            ...
            case "!taxalimite":
                ...
                if (novoEstado) {
                    ...
                    var qtd_max_minuto = args[1],
                        tempo_bloqueio = args[2]
            ...
            case "!mudarlimite":
                ...
                var tipo = args[1].toLowerCase(),
                    qtd = args[2]
                if (qtd != -1)
                    if (isNaN(qtd) || qtd < 5) return await client.reply(from, msgs_texto.admin.mudarlimite.invalido, id)
            ...
            case "!tipos":
                var tipos = botInfo().limite_diario.limite_tipos,
                    respostaTipos = ''
            ...
            case '!bctodos':
                if (args.length === 1) return await client.reply(from, erroComandoMsg(command), id)
                var mensagem = body.slice(9).trim(),
                    chats = await client.getAllChats(),
                    bloqueados = await client.getBlockedIds()
            ...
            case '!bccontatos':
                ...
                var mensagem = body.slice(12).trim(),
                    chats = await client.getAllChats(),
                    grupos = await client.getAllGroups(),
                    bloqueados = await client.getBlockedIds(),
                    qtdChatContatos = chats.length - grupos.length
            ...
            case '!bcgrupos':
                if (args.length === 1) return client.reply(from, erroComandoMsg(command), id)
                var mensagem = body.slice(10).trim(),
                    grupos = await client.getAllGroups()
            ...
            case "!grupos":
                var grupos = await client.getAllGroups(),
                    resposta = criarTexto(msgs_texto.admin.grupos.resposta_titulo, grupos.length)
                for (var grupo of grupos) {
            ...
            case '!restart':
                await client.reply(from, "Reiniciando Bot com sucesso!")
                await client.restart()
                break
            case "!ping":
                var os = require('os')
                var tempoResposta = (moment.now() / 1000) - t
                var memoriaTotal = os.totalmem() / 1024000000,
                    memoriaUsada = (os.totalmem() - os.freemem()) / 1024000000
                ...
                var chatContatos = await client.getAllContacts(),
                    chatGrupos = await client.getAllGroups()
